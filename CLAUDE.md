...
3. **Plan.** With `--plan`, `plan-file.ts` loads a YAML/JSON plan (natural-language or explicit steps), and `JevClient.classifySteps` classifies the natural-language steps as `click`/`fill`/`wait`/`submit` before the browser launches, rejecting `compound` steps. Without it, `workflow-plan.ts:createWorkflowPlan` builds a `WorkflowPlan` (navigate_section → click → fill_inputs → final-impact click → assert) with **Spanish-language regexes** over the prompt. `expectations.ts:inferExpectations` works the same way, and changing the wording those regexes match changes behavior. Both sources become `PlanStep[]` (`plan-steps.ts`), executed with an explicit cursor.
4. **Step loop** (up to `maxSteps`):
   ...
   - The loop stops when the cursor passes the last step (both modes; Jev is never asked for actions outside a step) or on the first non-`succeeded` record. Prompt runs also stop right after a `highImpact` click, and fail if `--max-steps` ends them early; plan runs continue until the last step, then fail if a step is incomplete or any input was not used.
   - A click is high impact on a submit/final-impact step or when the control name matches `jev.ts:HIGH_IMPACT_WORDS` (Spanish and English submission verbs).
5. **Expectations** come from `--expect '[not-]<kind>:<arg>'` (parsed in `expectation-spec.ts`), the legacy `--expect-*` aliases, and prompt inference. They are collected into the plan's `assert` step. `expectation-check.ts` verifies them live at the end with Playwright `expect` (5-second retries, negated ones last) and renders the same assertions for codegen. In dry-run they are marked `planned`. Any failed action or expectation makes the exit code `1`. Expectations are never sent to Jev.
