...
4. **Step loop** (up to `maxSteps`):
   ...
   - The loop stops on the first non-`succeeded` record. Prompt runs also stop right after a `highImpact` click; plan runs continue until the last step, then fail if any input was not used.
