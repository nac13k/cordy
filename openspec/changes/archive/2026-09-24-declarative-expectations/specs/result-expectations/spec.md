...
`--expect-visible X` SHALL behave exactly like `--expect 'text:X'`, and `--expect-button X` like `--expect 'button:X'`. `--expect-url X` SHALL require the URL to equal `X` exactly. Expectations inferred from the prompt SHALL continue to be added. Duplicate expectations (same kind, negation, and argument) SHALL be evaluated once.
