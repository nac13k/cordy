...
A click SHALL be high impact when its step is a `submit` step or when the element name matches Cordy's existing high-impact rule. High-impact clicks MUST require `--approve` in both plan mode and prompt (regex) mode. Without it, the click is recorded as `blocked` with the reason `requires --approve` and nothing is clicked. The existing guard against clicking while visible text fields are still empty SHALL apply to them. In plan mode, the run SHALL continue with the following steps after a high-impact click and stop only when the last step is complete or an action does not succeed.
...
- **WHEN** a prompt-derived run reaches its final `simular` click without `--approve`
