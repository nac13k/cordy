...
Without `--plan`, Cordy SHALL keep deriving the plan from the prompt as before. When no step can be derived, Cordy MUST exit with code 1 and a message suggesting `--plan`, instead of a raw schema error.

#### Scenario: Prompt with no derivable steps
- **WHEN** the user runs `cordy "Go to the Loans section and click Simulate"` with no inputs and no plan
- **THEN** Cordy exits with code 1 and a message that no plan could be derived from the prompt and that `--plan` can be used
