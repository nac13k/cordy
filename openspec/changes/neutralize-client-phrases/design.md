...
- [GitHub still serves the old commits through merged PRs #1 and #2 (`refs/pull/*`), cached views, and forks] → A force push cannot update these refs. After the push, ask GitHub Support to remove the cached views and dereference the old commits, listing the PRs and the old commit hashes. The repository is **public**, so these old commits stay publicly reachable (for example through the PR pages and `refs/pull/1/head`, `refs/pull/2/head`) until GitHub Support purges them; send the request right after the push. **Owner decision (2026-09-24):** no Support request for now. The owner will delete and recreate the repository manually, pushing only the rewritten refs, and reconfigure the npm publishing secrets. Until then the old commits stay reachable through the PR refs and direct commit URLs.
