...
export async function observePage(page: Page, task: string, observationId: string, recentActions: BrowserState['recentActions'] = [], workflow?: BrowserState['workflow']): Promise<BrowserState> {
  const elements = (await page.locator('input, textarea, select, button, a[href], [role="button"], [role="link"], [role="checkbox"], [role="radio"]').evaluateAll(nodes => nodes.map((node, index) => {
    ...
    const role = el.getAttribute('role') || (el.tagName.toLowerCase() === 'a' ? 'link' : el.tagName.toLowerCase() === 'button' ? 'button' : el.tagName.toLowerCase() === 'select' ? 'combobox' : 'textbox');
