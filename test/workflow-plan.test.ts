...
describe('workflow plan', () => {
  it('creates ordered navigation, fill, final click and result assertions', () => {
    ...
    expect(plan.steps.map(step => step.kind)).toEqual(['navigate_section', 'fill_inputs', 'click', 'assert']);
    expect(plan.steps[0]).toMatchObject({ kind: 'navigate_section', target: 'cotizador de envios' });
    expect(plan.steps[1]).toMatchObject({ kind: 'fill_inputs', inputKeys: ['peso', 'monto'] });
    expect(plan.steps[2]).toMatchObject({ kind: 'click', target: 'simular', finalImpact: true });
    expect(plan.steps[3]).toMatchObject({ kind: 'assert' });
    if (plan.steps[3].kind === 'assert') {
      expect(plan.steps[3].expectations).toContainEqual({ kind: 'button', name: 'guardar cotización' });
    }
  });
